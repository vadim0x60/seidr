...
branching_factor = {
    "lexicase": {
        ...
        "humaneval": {
            "gpt-3.5-turbo": {"C++": [2], "Python": [4]},
            "llama3": {"C++": [10], "Python": [10]},
        },
    },
    "no_lexicase": {
        "psb2": {
            ...
            "llama3": {"C++": [2, 4, 16, 1, 10, 100], "Python": [2, 4, 16, 1, 10, 100]},
        },
        "humaneval": {
            "gpt-3.5-turbo": {
                "C++": [2, 4, 16, 1, 10, 100],
...
dataset_id = {
    "psb2": 1,
    "humaneval": {
        "lexicase": {"Python": 2, "C++": 3},
        "no_lexicase": {"Python": 4, "C++": 5},
    },
}
...
number_of_experiments = {
    ...
    "humaneval": {
        "no_lexicase": 164
        * len(branching_factor["lexicase"]),  # only one language at a time
        "lexicase": 164,
    },
}
...
def update_sbatch_file(
    ...
    sbatch_template_file = (
        "scripts/tests/run_1_psb2_llama3_8b.sbatch"
        if "llama" in model_name
        else "scripts/tests/run_1_psb2_gpt4o.sbatch"
    )
    ...
    with open(sbatch_template_file, "r") as f:
        ...
        sbatch_lines[2] = (
            f"#SBATCH --partition={queue}          # partition (queue, see info about the queues below)\n"
        )
        sbatch_lines[8] = (
            f"#SBATCH --array=1-{number_of_experiments[dataset][lexicase_tag]}%5\n"
        )
        ...
        if dataset == "humaneval":
            ...
            sbatch_script = Path(
                f"scripts/{model_name}/{dataset}/run_{run}_{dataset}_{model_name}_{lexicase_tag}_{queue}.sbatch"
            )
...
if __name__ == "__main__":
    ...
    for run in range(1, runs + 1):
        for model_name, model_name_folder in zip(
            ["gpt-3.5-turbo", "llama3"], ["gpt3_5", "llama3_8b"]
        ):
            lexicase_tag = "lexicase" if lexicase_selection else "no_lexicase"
            offset = int(
                1000 * (run * 100 + model_id[model_name] * 10 + dataset_id[dataset])
            ...
            # lexicase
            output_file = (
                f"./config/{dataset}/{model_name_folder}/experiments_"
                f"{dataset}_{model_name_folder}_"
                f"run_{run}_offset_{offset}_{lexicase_tag}_"
            ...
            update_sbatch_file(
                model_name, output_file, dataset, lexicase_tag, run, offset
            )
    ...
    for run in range(1, runs + 1):
        for model_name, model_name_folder in zip(
            ...
            for language, language_tag in zip(["C++", "Python"], ["cpp", "py"]):
                offset = int(
                    1000
                    * (
                        run * 100
                        + model_id[model_name] * 10
                        + dataset_id[dataset][lexicase_tag][language]
                    )
                ) * (1 + 9 * int(lexicase_selection))
                ...
                experiments = [
                    ...
                    for bf in branching_factor[lexicase_tag][dataset][model_name][
                        language
                    ]
                ...
                output_file = (
                    ...
                    + "_".join(
                        [
                            str(s)
                            for s in branching_factor[lexicase_tag][dataset][
                                model_name
                            ][language]
                        ]
                    )
